...
const Occuptions = () => {
  ...
  const keys = ['name', 'image', 'customer_id', 'description'];
  ...
  useEffect(() => {
    const fetchData = async () => {
      const url = `${process.env.NEXT_PUBLIC_BASE_API_URL}/auth/occupation`;
      ...
      try {
        ...
        setRows(
          occupation.map((user) => ({
            ...
            descrirption: user.descrirption,
            image: user.image,
          }))
        );
      } catch (error) {
  ...
  const handleDataGridUpdate = async (updatedData, id) => {
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_API_URL}/auth/occupation/${id}`,
    ...
    <section className="px-8 pb-7">
      <div className="flex items-center flex-wrap justify-between my-3">
        <div>
          ...
          <p className="text-sm text-gray-400">Occuptions List</p>
        </div>
        <Link href="/dashboard/add-new-holiday">
          <ButtonComponent
            icon={<IoMdAdd />}
            content="Add new Occuption"
            buttonType="filled"
